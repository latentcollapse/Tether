# Tether Bootstrap — Gemini

**Your agent name is `gemini`.  You have Tether MCP tools.  Use them.**
...
Common recipients: `claude`, `kilo`, `matt`
...
```
tether_collapse(table="gemini-notes", data={"key": "value"})   # → returns a handle
tether_resolve(handle="h&l_gemini-notes_abc123")               # → returns the data
tether_snapshot(table="gemini-notes")                          # → all entries in table
```
